...
async def change_status():
    statuses = "kgb!help", "версия 1.7", "на {} серверах!", "SLAVA KPSS!"
    index = 0
...
async def help(ctx, command=None):
    ...
    if command is None:
        embed = discord.Embed(title="Лист команд:", description=f"Всего команд: {num_commands-3}\n***PREFIX*** - `kgb!`", color = discord.Colour(0x000000))
        embed.add_field(name = "📃Для просмотра иформации:", value="`server` `channel` `category` `role` \n`user` `avatar` `seekuser` `seekserver`", inline=False)
        embed.add_field(name = "🎮Развлечение:", value="`cat` `dog` `fox` `ball` `coin` `hack` `hackp`\n`comment` `rand` `wiki` `tt` `tc` `quote` \n`shtr` `comrade` `horny`", inline=False)
        embed.add_field(name = "🎭Рп:", value="`hug` `kiss` `hit` `lick` `hi` `pet`", inline=False)
        embed.add_field(name = "🛡️Для модерации:", value="`welcome` `clear` `kick` `ban` `unban` \n`banlist` `warn` `unwarn` `warnings`", inline=False)
        embed.add_field(name = "🛠Остальное:", value="`verlist` `killbot` `cipher` `thank` \n`invite` `ping` `poll` `configwarn` `null`", inline=False)
        embed.add_field(name = "Что бы проверить что делает команда \nНапишите: ", value='`kgb!help (команда)`', inline=False)
    ...
    for guild in kgb.guilds:
        for member in guild.members:
            if query.lower() in member.display_name.lower() or query.lower() in member.name.lower():
                users_found.add(f"{member.name}#{member.discriminator}")
...
async def seekserver(ctx, *, user_name):
    if isinstance(ctx.channel, discord.DMChannel):
      return
    guild_seek = None
